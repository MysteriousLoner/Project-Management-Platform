...
import { SUBTASK_STATUSES } from "@/lib/types";
    ...
    const item = await transaction(async (client) => {
      ...
      const result = await client.query(
        `INSERT INTO work_items (
          ...
          assignee_id, created_by_id, updated_by_id, blocked_reason, estimated_completion_date
  ...
  try {
    ...
    const item = await transaction(async (client) => {
      ...
      await audit(client, {
        ...
        fieldName: patch.status && patch.status !== current.status ? "status" : null,
